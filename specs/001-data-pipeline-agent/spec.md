...
- **FR-107**: 质量门禁的结论 MUST 独立于阶段闸门记录,MUST NOT 推进阶段闸门——门禁判定的是"这一次运行的结果是否可用",阶段闸门判定的是"这个版本是否可以上线",两者含义不同且重跑时前者需重新判定而后者不变。切换线上指向 MUST 同时满足两项:阶段闸门已达发布就绪,且本次运行的门禁结论为通过。该结论 MUST 只由门禁工具的真实返回值写入,MUST NOT 由模型自述写入。
