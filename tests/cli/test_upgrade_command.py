...
def _uv_registry_info() -> _InstalledWheelInfo:
    """A registry uv-tool install → ``uv tool upgrade omniagentkit`` (runnable)."""
...
def test_upgrade_runs_installer_and_drains_first(
    ...
    assert ran == ["uv tool upgrade omniagentkit"]
...
def test_upgrade_pre_passes_prerelease_flag_to_installer(
    ...
    assert result.exit_code == 0, result.output
    assert ran == ["uv tool upgrade omniagentkit --prerelease allow"]
...
def test_nightly_suggestion_preserves_and_unions_extras() -> None:
    ...
    assert suggestion.command == (
        ...
        "git+https://github.com/omnigent-ai/omnigent@v0.9.0.dev20260804#egg=omniagentkit[all,server]"
    )
